...
from curve_dao.contract import get_abi_with_4bytes, get_contract
...
def prepare_vote_script(target: Dict, actions: List[Tuple]) -> str:
    ...
    agent = get_contract(target["agent"])
...
def make_vote(target: Dict, actions: List[Tuple], description: str, vote_creator: str):
    ...
    aragon = get_contract(target["voting"])
...
def decode_vote_script(script):
    ...
    while idx < len(script):
        ...
        target, abi_with_4bytes = get_contract(script[idx : idx + 20])
        idx += 20
        ...
        fn, inputs = decode_input(abi_with_4bytes, calldata)
        agent = None

        # print decoded vote:
        if calldata[:4].hex() == "b61d27f6":
            agent = target
            # target = ape.Contract(inputs[0])
            target, abi_with_4bytes = get_contract(inputs[0])
            fn, inputs = decode_input(abi_with_4bytes, inputs[2])
            inputs_with_names = get_inputs_with_names(fn, inputs)
            formatted_inputs = format_fn_inputs(inputs_with_names)
            formatted_output = (
                f"Call via agent: [yellow]{agent.address}[/]\n"
                f" ├─ [bold]To[/]: [green]{target.address}[/]\n"
